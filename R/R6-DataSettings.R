...
DataSettings <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return `self`
    initialize = function(connectionDetails, cdmDatabaseSchema, resultSchema, cohortTable) {
    ...
    validate = function() {
      ...
      checkmate::assertCharacter(x = private$connectionDetails$dbms, len = 1, null.ok = FALSE, add = errorMessages)
      checkmate::assertCharacter(private$cdmDatabaseSchema, null.ok = FALSE, len = 1, add = errorMessages)
      checkmate::assertCharacter(private$resultSchema, null.ok = FALSE, len = 1, add = errorMessages)
