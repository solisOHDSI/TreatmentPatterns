...
tempOutput <- file.path(tempdir(), "output")
tempTemp <- file.path(tempdir(), "temp")
...
```{r addPathwayAnalysis, eval=TRUE}
...
4. **cohortSettings**: `cohortSettings` object.
...
### Sunburst plot
```{r eval=FALSE}
sunburstData <- TreatmentPatterns::formatSunburstData(
  filePath = file.path(saveSettings$outputFolder, "freqPerYear.csv"),
  studyName = "Viral_Sinusitis",
  indexYear = "no year"
)
...
TreatmentPatterns::createSunburstPlot(
  sunburstData,
  folder = file.path(saveSettings$outputFolder, "/"),
  fileName = "sunburstPlot.html"
)

webshot2::webshot(
  url = "output/sunburstPlot.html",
  file = "output/sunburstPlot.png",
  selector = "svg", vwidth = 1200
)
```
![sunburstPlot](../man/figures/sunburstplot.png)

```{r sankeyDiagram, eval=FALSE}
sankeyData <- read.csv(file.path(saveSettings$outputFolder, "freqPerYear.csv"))
...
exportFigure(
  filePath = "output/sankeyDiagram.html",
  outputPath = "output/sankeyDiagram.png",
  selector = "svg"
)
```
![sankeyDiagram](../man/figures/sankeydiagram.png)
